...
resource "null_resource" "python_script_purchase_phone_number" {
  depends_on = [azurerm_communication_service.communication_service]
  provisioner "local-exec" {
    command = "${path.module}\\..\\.venv\\Scripts\\python.exe ${path.module}\\purchase_phone_number.py --connection-string ${azurerm_communication_service.communication_service.primary_connection_string}"
  }
  triggers = {
    always = "${timestamp()}"
  }
